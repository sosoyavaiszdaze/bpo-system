...
def _check_technical(camps, pixel_status):
    ...
    results.append(_r("T01", ps.get("pixel_installed", False), "アカウント全体",
                       "" if ps.get("pixel_installed") else "TikTok Pixel 未設置"))

    # T-TC2: ttclid パスバック
    results.append(_r("T02", ps.get("ttclid_passback", False), "アカウント全体",
...
def _check_creative(camps, t_t):
    ...
    for camp in camps:
        ...
        if ad_count > 0 and not has_video:
            results.append(_r("T36", False, name, "TikTokに非動画クリエイティブ — 動画必須"))

        # T-CR2: 動画尺（9-15秒推奨）
        avg_duration = camp.get("avg_video_duration", 0)
        if avg_duration > 0:
            results.append(_r("T16", 9 <= avg_duration <= 30, name,
                              "" if 9 <= avg_duration <= 30 else f"動画尺 {avg_duration:.0f}秒 (推奨: 9-30秒)"))

        # T-CR3: 動画完視聴率 ≥15%
        vcr = camp.get("video_completion_rate", 0)
        if vcr > 0:
            results.append(_r("T37", vcr >= vcr_min, name,
        ...
        if is_spark:
            results.append(_r("T23", True, name, "Spark Ads 使用中 ✓"))

        # T-CR5: Hook率（最初3秒の維持率）
        hook_rate = camp.get("hook_rate", 0)
        if hook_rate > 0:
            results.append(_r("T19", hook_rate >= 30, name,
        ...
        if has_overlay is not None:
            results.append(_r("T38", has_overlay, name,
    ...
    if total_ads > 0 and len(camps) > 0:
        ...
        results.append(_r("T14", avg_ads >= 5, "全TikTok",
    ...
    results.append(_r("T15", has_smart, "全TikTok",
...
def _check_bidding_learning(camps, t_t):
    ...
    for camp in camps:
        ...
        if cv < daily_min and cost > 0:
            results.append(_r("T06", False, name,
...
def _check_structure(camps, t_t):
    ...
    for camp in camps:
        ...
        results.append(_r("T40", has_structure, name,
    ...
    for obj, group in obj_groups.items():
        if len(group) > 3:
            results.append(_r("T10", False, f"{obj}グループ",
    ...
    if len(camps) > 1:
        results.append(_r("T41", has_retarget and has_prospecting, "全TikTok",
    ...
    if app_camps:
        results.append(_r("T42", has_ios and has_android, "全TikTok",
    ...
    for camp in camps:
        ...
        if adgroup_count > max_adgroups:
            results.append(_r("T43", False, camp["campaign"],
    ...
    for i, c1 in enumerate(camps):
        for c2 in camps[i + 1:]:
            ...
            if (c1.get("campaign_type") == c2.get("campaign_type") and
                ...
                results.append(_r("T44", False, f"{c1['campaign']} / {c2['campaign']}",
...
def _check_campaign_config(camps, t_t):
    ...
    for camp in camps:
        ...
        if opt_event and opt_event.lower() in ("impression", "click"):
            results.append(_r("T03", False, name,
        ...
        if delivery_type == "dayparting" and camp.get("conversions", 0) < 20:
            results.append(_r("T45", False, name,
    ...
    for camp in camps:
        ...
        if "auto" in bidding.lower() and "auto" in targeting.lower():
            results.append(_r("T46", True, camp["campaign"],
                              "自動入札+自動ターゲティング ✓"))
        elif "manual" in bidding.lower() and "manual" in targeting.lower():
            results.append(_r("T46", False, camp["campaign"],
    ...
    for camp in camps:
        ...
        if pangle is True:
            ...
            if placement_performance < 0.5:
                results.append(_r("T30", False, camp["campaign"],
