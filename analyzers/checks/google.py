...
def _check_structure(camps, g_t):
    ...
    for camp in camps:
        ...
        results.append(_r("G01", has_structure, name,
    ...
    for camp in search_camps:
        ...
        if kw_count > 0:
            ...
            results.append(_r("G03", avg_kw <= 15, camp["campaign"],
    ...
    for ct, count in type_counts.items():
        if count > 5:
            results.append(_r("G04", False, f"{ct}タイプ",
    ...
    if search_camps:
        results.append(_r("G05", has_brand and has_nonbrand, "全Search",
    ...
    if pmax_camps and search_camps:
        ...
        results.append(_r("G07", pm_has_neg, "アカウント全体",
    ...
    for camp in camps:
        ...
        if budget > 0 and cost > 0:
            util = cost / budget
            results.append(_r("G08", util < 0.95, camp["campaign"],
    ...
    if total_cost > 0 and len(camps) > 1:
        for camp in camps:
            ...
            if share > 50 and (roas < 1 or cv == 0):
                results.append(_r("G09", False, camp["campaign"],
    ...
    for camp in camps:
        ...
        if geo_setting and "interest" in geo_setting.lower():
            results.append(_r("G11", False, camp["campaign"],
    ...
    for camp in search_camps:
        ...
        if search_partners is True:
            results.append(_r("G12", False, camp["campaign"],
...
def _check_negative_kw(camps, g_t):
    ...
    for camp in search_camps:
        ...
        if not last_review:
            results.append(_r("G13", False, name, "検索語句レビュー日不明 — 定期レビュー推奨"))

        # G14: ネガティブKWリスト存在
        neg_lists = camp.get("negative_keyword_lists", [])
        results.append(_r("G14", len(neg_lists) > 0, name,
                          "" if neg_lists else "ネガティブKWリスト未設定"))

        # G15: 共有ネガKWリスト
        shared_neg = camp.get("shared_negative_lists", [])
        results.append(_r("G15", len(shared_neg) > 0, name,
        ...
        if waste_clicks > waste_threshold:
            results.append(_r("G16", False, name,
        ...
        if "broad" in [m.lower() for m in match_types] and "manual" in bidding.lower():
            results.append(_r("G17", False, name, "Broad Match + Manual CPC: Smart Bidding推奨"))

        # G-WS1: ゼロCV KW with cost
        if camp.get("conversions", 0) == 0 and camp.get("cost", 0) > 0 and camp.get("keyword_count", 0) > 0:
            results.append(_r("G-WS1", False, name,
...
def _check_keywords_qs(camps, g_t):
    ...
    for camp in search_camps:
        ...
        if avg_qs is not None:
            results.append(_r("G20", avg_qs >= 5, name,
                              "" if avg_qs >= 5 else f"平均QS {avg_qs:.1f} (推奨: ≥5)"))

        # G21: QS≤3 比率
        qs_low_ratio = camp.get("qs_low_ratio", 0)
        if qs_low_ratio > 0:
            results.append(_r("G21", qs_low_ratio <= 20, name,
        ...
        if ectr_below > 0:
            results.append(_r("G22", ectr_below <= 30, name,
        ...
        if ar_below > 0:
            results.append(_r("G23", ar_below <= 30, name,
        ...
        if lp_below > 0:
            results.append(_r("G24", lp_below <= 30, name,
        ...
        if kw_count > 0 and zero_imp_kw > 0:
            ...
            results.append(_r("G-KW1", ratio <= 30, name,
...
def _check_ads_assets(camps, g_t):
    ...
    for camp in camps:
        ...
        if ct == "search":
            results.append(_r("G26", ad_count >= 1, name,
        ...
        if headline_counts:
            ...
            results.append(_r("G27", min_hl >= 8, name,
        ...
        if desc_counts:
            ...
            results.append(_r("G28", min_desc >= 4, name,
        ...
        if ad_strengths:
            ...
            results.append(_r("G29", len(poor) == 0, name,
        ...
        if ct == "pmax":
            ...
            results.append(_r("G31", len(problems) == 0, name,
        ...
        if ct in ("display", "video", "demand_gen", "pmax"):
            ...
            results.append(_r("G32", has_video, name,
...
def _check_bidding(camps, g_t):
    ...
    for camp in camps:
        ...
        if ct in ("search", "shopping"):
            ...
            results.append(_r("G36", is_smart, name,
        ...
        if target_cpa > 0 and actual_cpa > 0:
            ...
            if ratio > 1.5:
                results.append(_r("G37", False, name,
        ...
        if camp.get("learning_phase"):
            results.append(_r("G38", False, name, "学習フェーズ中: CV不足"))
        ...
        if limited or (budget > 0 and cost >= budget * 0.95):
            results.append(_r("G39", False, name,
        ...
        if bidding == "manual_cpc" and ct not in ("display",):
            cv = camp.get("conversions", 0)
            if cv >= 30:
                results.append(_r("G40", False, name,
...
def _check_conversion_tracking(camps, g_t):
    ...
    results.append(_r("G43", first.get("enhanced_conversions", False), "アカウント全体",
    ...
    if consent_mode is not None:
        results.append(_r("G45", consent_mode, "アカウント全体",
                          "" if consent_mode else "Consent Mode v2 未実装 — EU/UK向けの場合必須"))
    else:
        results.append(_r("G45", False, "アカウント全体", "Consent Mode v2 ステータス不明"))

    # G47: マクロ/マイクロ CV分離
    cv_actions = first.get("conversion_action_count", 0)
    primary_actions = first.get("primary_conversion_actions", 0)
    if cv_actions > 0:
        results.append(_r("G47", primary_actions > 0 and primary_actions < cv_actions, "アカウント全体",
...
def _check_extensions(camps, g_t):
    ...
    if camps:
        ...
        results.append(_r("G56", len(audiences) > 0, "アカウント全体",
                          "" if audiences else "オーディエンスセグメント未設定"))

        # G57: Customer Match
        cm = first.get("customer_match_enabled", False)
        results.append(_r("G57", cm, "アカウント全体",
        ...
        for dpc in display_pmax:
            ...
            results.append(_r("G58", exclusions > 0, dpc["campaign"],
...
def _check_pmax(camps, g_t):
    ...
    for camp in pmax_camps:
        ...
        results.append(_r("G-PM1", len(audience_signals) > 0, name,
        ...
        if ad_strengths:
            ...
            results.append(_r("G-PM2", len(good) > 0, name,
...
def _check_advanced(camps, g_t):
    ...
    for camp in camps:
        ...
        if ai_max is not None:
            results.append(_r("G-AI1", True, camp["campaign"],
    ...
    for camp in dg_camps:
        ...
        results.append(_r("G-DG1", has_video and has_image, name,
        ...
        if freq > 3 and freq_cap == 0:
            results.append(_r("G-DG3", False, name,
    ...
    for camp in vac_camps:
        results.append(_r("G-DG2", False, camp["campaign"],
    ...
    for camp in ctv_camps:
        results.append(_r("G-CTV1", True, camp["campaign"],
